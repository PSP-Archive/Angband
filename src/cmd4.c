...
int feat_order(int feat)
{
	...
	switch (f_ptr->d_char)
	{
		...
		default:
		{
			if (isdigit((unsigned char) f_ptr->d_char)) return 7;
			return 8;
		}
	}
}
...
void do_cmd_save_screen(void)
{
	...
	int index = 0;
	int file;
	while (1) { // find an available screenshot number
		sprintf(tmp_val, "shot%03d.bmp", index);
		path_build(path, sizeof(path), ANGBAND_DIR_USER, tmp_val );
		if ((file = sceIoOpen(path, PSP_O_RDONLY, 0777)) < 0) break;
		sceIoClose(file);
		index++;
	}
	
	/* Ask for a file with default being name generated above */
	if (!get_string("File: ", tmp_val, sizeof(tmp_val))) return;
	
	/* Build the filename */
	path_build(path, sizeof(path), ANGBAND_DIR_USER, tmp_val );
	
	unsigned char bmpHeader24[] = { 0x42, 0x4d, 0x38, 0xfa, 0x05, 0x00, 0x00, 0x00, 0x00, 0x00, 0x36, 0x00, 0x00, 0x00, 0x28, 0x00, 0x00, 0x00, 0xe0, 0x01, 0x00, 0x00, 0x10, 0x01, 0x00, 0x00, 0x01, 0x00, 0x18, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
	unsigned char buffer[480*3];
	int bufferIndex, i, e;

	file = sceIoOpen(path, O_CREAT | O_TRUNC | O_RDWR, 0777); 

	// write bmp header
	sceIoWrite(file,bmpHeader24,54);
   
	// write bmp data
	unsigned long* vptr0 =((unsigned long*)(0x40000000 | (unsigned long)sceGeEdramGetAddr()) + 271 * 512);
	for(i=0; i<272; i++, vptr0-=512)
	{      
		unsigned long* vptr = vptr0;
		for(e=0, bufferIndex = 0; e<480; e++)
		{
			unsigned long p = *vptr;
         
			buffer[bufferIndex++] = (p>>16)&0xff;   
			buffer[bufferIndex++] = (p>>8)&0xff;
			buffer[bufferIndex++] = (p)&0xff;
